...
void map_build_implementation_map(Map* implementationMap, Map* methodMap, Map*parentMap, char* id){
    Map* parent = map_get_map_by_id(parentMap, id);
    Map* method = map_get_map_by_id(methodMap, id);
    int i, j, c = 0;
    int l = method->numchild;
    int pl = 0;
    if (parent){
        parent = map_get_map_by_id(implementationMap, (char*)parent->data);
        pl= parent->numchild;
    }
    ImpMethod* data = malloc(sizeof(ImpMethod)*(l+pl));
    ImpMethod* md;
    if (parent){
        md = (ImpMethod*)parent->data;
        for (i = 0; i < pl; i++){
            data[i] = md[i];
            c++;
        }
        md = (ImpMethod*)method->data;
        int override;
        for (i = 0; i < l; i++){
            override = 0;
            for (j = 0; j < pl; j++){
                if (typecheck_is_method_override(&data[j], &md[i])){
                    override = 1;
                    data[j] = md[i];
                    break;
                }
    ...
    map_insert_sorted(implementationMap, id, c, method->linenum, data);
    return;
}
void map_build_class_map(Map* classMap, Map* attributeMap, Map*parentMap, char* id){
    Map* parent = map_get_map_by_id(parentMap, id);
    Map* attribute = map_get_map_by_id(attributeMap, id);

    int i, j, c = 0;
    int l = attribute->numchild;
    int pl = 0;
    if (parent){
        parent = map_get_map_by_id(classMap, (char*)parent->data);
        pl= parent->numchild;
    }
    Attribute* data = malloc(sizeof(Attribute)*(l+pl));
    Attribute* md;
    if (parent){
        md = (Attribute*)parent->data;
        for (i = 0; i < pl; i++){
            data[c] = md[i];
            c++;
        }
    }
    md = (Attribute*)attribute->data;
    for (i = 0; i < l; i++){
        data[c] = md[i];
        c++;
    }
    map_insert_sorted(classMap, id, c, attribute->linenum, data);
    return;
}